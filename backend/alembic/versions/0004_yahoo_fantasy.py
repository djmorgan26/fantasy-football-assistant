...
Numbered 0004, not 0003: this landed alongside `0003_league_members` and both
declared revision "0003", which left the chain with two heads and made
`alembic upgrade head` ambiguous. Nothing about the schema change moved, only
...
def upgrade() -> None:
    ...
    # The league row carries one Yahoo identity, which is only ever the owner's.
    op.add_column("league_members", sa.Column("yahoo_guid", sa.String(length=255), nullable=True))


def downgrade() -> None:
    op.drop_column("league_members", "yahoo_guid")
