    ...
    <div className={cn('flex flex-col', fullWidth && 'w-full', className)}>
      ...
      <Listbox value={value} onChange={onChange} disabled={disabled}>
        <div className="relative">
          <Listbox.Button
            className={cn(
              'relative w-full cursor-pointer rounded-lg border border-border bg-surface-raised py-2 pl-3 pr-10 text-left text-sm text-fg shadow-sm',
          ...
          >
            ...
            <span className="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-2">
              <ChevronUpDownIcon className="h-5 w-5 text-fg-subtle" aria-hidden="true" />
            </span>
          </Listbox.Button>
          <Transition
            as={Fragment}
            ...
            leaveTo="opacity-0"
          >
            <Listbox.Options className="absolute z-50 mt-1 max-h-60 w-full overflow-auto rounded-lg border border-border bg-surface-raised py-1 text-sm shadow-elevation-3 focus:outline-none">
              {options.map((opt) => (
                <Listbox.Option
                  key={opt.value}
                  value={opt.value}
